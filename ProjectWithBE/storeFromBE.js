...
function showPagination(paginationInfo) {
    ...
    if (paginationInfo.hasPreviousPage) {
        ...
        previousPageBtn.addEventListener('click', (event) => {
            axios.get(`http://localhost:3000/products?page=${paginationInfo.previousPage}`)
                .then(response => {
                    showProducts(response.data.pageProducts);
                    showPagination(response.data.paginationInfo);
                })
    ...
    currentPageBtn.addEventListener('click', (event) => {
        axios.get(`http://localhost:3000/products?page=${paginationInfo.currentPage}`)
            .then(response => {
                showProducts(response.data.pageProducts);
                showPagination(response.data.paginationInfo);
            })
    ...
    if (paginationInfo.hasNextPage) {
        ...
        nextPageBtn.addEventListener('click', (event) => {
            axios.get(`http://localhost:3000/products?page=${paginationInfo.nextPage}`)
                .then(response => {
                    showProducts(response.data.pageProducts);
                    showPagination(response.data.paginationInfo);
                })

...
function showPagination(paginationInfo) {
    ...
    if (paginationInfo.hasPreviousPage) {
        ...
        previousPageBtn.addEventListener('click', async (event) => {
            try {
                const response = await axios.get(`http://localhost:3000/products?page=${paginationInfo.previousPage}`)
                if (response.status === 200) {
                    showProducts(response.data.pageProducts);
                    showPagination(response.data.paginationInfo);
                } else {
    ...
    currentPageBtn.addEventListener('click', async (event) => {
        try {
            const response = await axios.get(`http://localhost:3000/products?page=${paginationInfo.currentPage}`)
            if (response.status === 200) {
                showProducts(response.data.pageProducts);
                showPagination(response.data.paginationInfo);
            } else {
    ...
    if (paginationInfo.hasNextPage) {
        ...
        nextPageBtn.addEventListener('click', async (event) => {
            try {
                const response = await axios.get(`http://localhost:3000/products?page=${paginationInfo.nextPage}`)
                if (response.status === 200) {
                    showProducts(response.data.pageProducts);
                    showPagination(response.data.paginationInfo);
                } else {
...
async function changeOfQuantity(event, id) {
    try {
        ...
        const response = await axios.patch(`http://localhost:3000/cart/${id}`, { quantity: input.value })
        if (response.status === 200) {
            createCart();
        } else {
